...
export default function Pferde() {
  ...
  const [form, setForm] = useState({ name: "", age: "", breed: "", use_type: "", notes: "" });
  ...
  const handleAdd = async () => {
    ...
    setForm({ name: "", age: "", breed: "", use_type: "", notes: "" });
    setShowForm(false);
    fetchHorses();
  };
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
            <div className="p-6 rounded-xl bg-card border border-border space-y-4">
              ...
              <button onClick={handleAdd} className="px-6 py-2.5 rounded-lg bg-primary text-primary-foreground text-sm font-medium hover:opacity-90 transition-opacity">Speichern</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {horses.map((horse, i) => (
          <motion.button key={horse.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
            ...
            className={`text-left p-5 rounded-xl border transition-colors ${selected?.id === horse.id ? "bg-primary/5 border-primary/30" : "bg-card border-border hover:border-primary/20"}`}
          >
            <div className="flex items-center gap-3 mb-3">
              <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center"><Heart size={18} className="text-primary" /></div>
              <div>
